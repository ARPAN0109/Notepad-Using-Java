...
public class notePadR implements ActionListener
{
    JFrame frm;
    JMenuBar mnubar;
    JScrollPane jsrlpane;
    JMenu fileMenu, editMenu, fontMenu, helpMenu;
    JMenu fontType, fontStyle, fontSize, bg ,txtclr;
    JMenuItem saveAs,open,close;
    JMenuItem cutItem, copyItem, pasteItem, selectAll;
    JMenuItem f1,f2,f3,f4,f5,fs10,fs20,fs30,fs40,fs50, red, blue, green, white, plain, italics, bold ,r,g,b,bl,wt;
    JTextArea txtarea;
    //FrontEnd Designing
    notePadR()//Constructor to initialize class variables
    {
        frm = new JFrame();
        frm.setTitle(" Simple Notepad ");

        mnubar = new JMenuBar();
        mnubar.setBounds(0, 0, 1800, 25);

        txtarea = new JTextArea();
        txtarea.setBounds(0, 25, 1800, 800);

        jsrlpane = new JScrollPane();
        jsrlpane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
        jsrlpane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        ...
        frm.add(mnubar);
        frm.add(txtarea);
        frm.add(jsrlpane, BorderLayout.CENTER);
        frm.setLayout(null);
        frm.setSize(1800, 800);
        frm.setVisible(true);
    ...
    public void actionPerformed(ActionEvent ae)
    {
        ...
        String x[] = {"Serif","Century Gothic","SansSerif","Monospaced","Courier",},nm="";
        int s[]={0,2,1},st=0;
        int f[]={10,20,30,40,50},fs=20;
        //font packs
        if (ae.getSource()==f1) {
